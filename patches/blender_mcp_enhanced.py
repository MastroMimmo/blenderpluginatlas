...
class BlenderMCPServer:
    ...
    def _execute_command_internal(self, command):
        ...
        # ── Custom: StableGen (auto-detect if installed) ──
        if hasattr(bpy.ops, "stablegen"):
            handlers.update({
                "stablegen_get_prefs": self.stablegen_get_prefs,
                "stablegen_set_prefs": self.stablegen_set_prefs,
                "stablegen_get_scene_props": self.stablegen_get_scene_props,
                "stablegen_set_scene_props": self.stablegen_set_scene_props,
                "stablegen_list_checkpoints": self.stablegen_list_checkpoints,
                "stablegen_queue_add": self.stablegen_queue_add,
                "stablegen_queue_process": self.stablegen_queue_process,
                "stablegen_queue_clear": self.stablegen_queue_clear,
            })

        # ── Custom: BlenderToDazStudio (auto-detect if installed) ──
        if hasattr(bpy.ops, "dazstudio"):
            handlers.update({
                "daz_merge_textures_all": self.daz_merge_textures_all,
                "daz_merge_textures_selected": self.daz_merge_textures_selected,
                "daz_export_all": self.daz_export_all,
                "daz_export_selected": self.daz_export_selected,
            })

        # ── Custom: Trellis 2 via ComfyUI REST API (no Blender addon) ──
        handlers.update({
            "trellis2_submit": self.trellis2_submit,
            "trellis2_status": self.trellis2_status,
        })

        # ── Custom: generic project I/O ──
        handlers.update({
            "import_gltf": self.import_gltf_file,
            "import_obj": self.import_obj_file,
            "import_fbx": self.import_fbx_file,
            "export_gltf": self.export_gltf_file,
            "export_fbx": self.export_fbx_file,
            "list_addons": self.list_addons,
        })

        # ── Custom: universal plugin discovery + execution ──
        # Questi handler rendono controllabile QUALSIASI plugin/addon installato
        # senza bisogno di scrivere handler specifici per ognuno.
        handlers.update({
            "inspect_addon": self.inspect_addon,
            "inspect_ops_namespace": self.inspect_ops_namespace,
            "call_operator": self.call_operator,
            "get_addon_prefs": self.get_addon_prefs,
            "set_addon_prefs": self.set_addon_prefs,
            "get_scene_prop": self.get_scene_prop,
            "set_scene_prop": self.set_scene_prop,
            "find_scene_props": self.find_scene_props,
            "list_operator_namespaces": self.list_operator_namespaces,
        })
    ...
    # ── StableGen handlers ──
    def _stablegen_prefs(self):
        return bpy.context.preferences.addons["bl_ext.user_default.stablegen"].preferences

    def stablegen_get_prefs(self):
        p = self._stablegen_prefs()
        return {k: str(getattr(p, k)) for k in
                ("server_address", "output_dir", "save_blend_file", "enable_debug",
                 "timeout_api", "timeout_mesh_gen", "timeout_ping", "server_online")}

    def stablegen_set_prefs(self, **kwargs):
        p = self._stablegen_prefs()
        for k, v in kwargs.items():
            if hasattr(p, k):
                setattr(p, k, v)
        bpy.ops.wm.save_userpref()
        return self.stablegen_get_prefs()

    def stablegen_get_scene_props(self):
        s = bpy.context.scene
        keys = [k for k in dir(s) if not k.startswith("_") and any(
            x in k.lower() for x in ("stablegen", "generation", "trellis2_generate_from", "sg_"))
            and not callable(getattr(s, k, None))]
        return {k: str(getattr(s, k, None))[:200] for k in keys}

    def stablegen_set_scene_props(self, **kwargs):
        s = bpy.context.scene
        for k, v in kwargs.items():
            if hasattr(s, k):
                setattr(s, k, v)
        return self.stablegen_get_scene_props()

    def stablegen_list_checkpoints(self):
        import urllib.request
        try:
            data = json.load(urllib.request.urlopen("http://127.0.0.1:8188/object_info/CheckpointLoaderSimple", timeout=5))
            return data["CheckpointLoaderSimple"]["input"]["required"]["ckpt_name"][0]
        except Exception as e:
            return {"error": str(e)}

    def stablegen_queue_add(self):
        bpy.ops.stablegen.queue_add(); return {"ok": True}

    def stablegen_queue_process(self):
        bpy.ops.stablegen.queue_process(); return {"ok": True}

    def stablegen_queue_clear(self):
        bpy.ops.stablegen.queue_clear(); return {"ok": True}

    # ── DAZ handlers ──
    def daz_merge_textures_all(self):
        bpy.ops.dazstudio.merge_textures_all_objects(); return {"ok": True}

    def daz_merge_textures_selected(self):
        bpy.ops.dazstudio.merge_textures_selected_objects(); return {"ok": True}

    def daz_export_all(self):
        bpy.ops.export_scene.dazstudio_all(); return {"ok": True}

    def daz_export_selected(self):
        bpy.ops.export_scene.dazstudio_selected(); return {"ok": True}

    # ── Trellis 2 (ComfyUI REST) ──
    def trellis2_submit(self, workflow_path=None, prompt=None):
        """Submit un workflow API-format (dict) o un path .json al server ComfyUI."""
        import urllib.request
        if workflow_path and not prompt:
    ...
    def trellis2_status(self, prompt_id=None):
        import urllib.request
        url = "http://127.0.0.1:8188/history/" + prompt_id if prompt_id else "http://127.0.0.1:8188/queue"
        return json.loads(urllib.request.urlopen(url, timeout=5).read().decode())

    # ── Project I/O ──
    def import_gltf_file(self, filepath):
        bpy.ops.import_scene.gltf(filepath=filepath); return {"ok": True, "path": filepath}

    def import_obj_file(self, filepath):
        bpy.ops.wm.obj_import(filepath=filepath); return {"ok": True, "path": filepath}

    def import_fbx_file(self, filepath):
        bpy.ops.import_scene.fbx(filepath=filepath); return {"ok": True, "path": filepath}

    def export_gltf_file(self, filepath, selected_only=False):
        bpy.ops.export_scene.gltf(filepath=filepath, use_selection=selected_only)
        return {"ok": True, "path": filepath}

    def export_fbx_file(self, filepath, selected_only=False, embed_textures=True):
        bpy.ops.export_scene.fbx(filepath=filepath, use_selection=selected_only,
                                  path_mode="COPY", embed_textures=embed_textures,
                                  mesh_smooth_type="FACE", add_leaf_bones=False)
        return {"ok": True, "path": filepath}

    def list_addons(self):
        return sorted(bpy.context.preferences.addons.keys())

    # ── Universal plugin discovery + execution ──
    # Questi handler espongono QUALSIASI plugin installato senza doverne scrivere uno per ognuno.

    def list_operator_namespaces(self):
        """Elenca tutti i namespace operatori (es. 'stablegen', 'mesh', 'dazstudio')."""
        return sorted([n for n in dir(bpy.ops) if not n.startswith("_")])

    def inspect_ops_namespace(self, namespace):
        """Elenca tutti gli operatori di un namespace (es. 'stablegen' → ['queue_add', 'queue_process', ...])."""
        ns = getattr(bpy.ops, namespace, None)
        if not ns:
            return {"error": f"namespace '{namespace}' non trovato"}
        ops = []
        for name in dir(ns):
            ...
            try:
                ...
                desc = rna.description if rna else ""
                props = {p.identifier: p.description for p in rna.properties if p.identifier != "rna_type"} if rna else {}
                ops.append({"name": name, "description": desc, "properties": props})
            except Exception as e:
                ops.append({"name": name, "error": str(e)})
        return {"namespace": namespace, "operators": ops}

    def call_operator(self, operator, params=None, execution_context=None):
        """Chiama un operatore arbitrario, es. 'stablegen.queue_process' con params={}."""
        params = params or {}
        parts = operator.split(".")
        if len(parts) != 2:
            return {"error": "operator must be '<namespace>.<name>', es. 'stablegen.queue_process'"}
        ns, name = parts
        ns_obj = getattr(bpy.ops, ns, None)
        if not ns_obj:
            return {"error": f"namespace '{ns}' non trovato"}
        op = getattr(ns_obj, name, None)
        if not op:
            return {"error": f"operator '{operator}' non trovato"}
        args = [execution_context] if execution_context else []
        try:
            result = op(*args, **params)
            return {"ok": True, "result": list(result) if hasattr(result, "__iter__") else str(result)}
        except Exception as e:
            import traceback
            return {"ok": False, "error": str(e), "traceback": traceback.format_exc()[-500:]}

    def _resolve_addon_key(self, name):
        """Matcha 'stablegen' → 'bl_ext.user_default.stablegen' se necessario."""
    ...
    def inspect_addon(self, name):
        """Info completa su un addon: preferenze, operatori collegati (via name match + source scan), metadata."""
        import os, re
        key = self._resolve_addon_key(name)
        if not key:
            return {"error": f"addon '{name}' non abilitato", "available": sorted(bpy.context.preferences.addons.keys())}
        addon = bpy.context.preferences.addons[key]
        prefs = addon.preferences
        result = {"key": key, "preferences": {}, "operator_namespaces": [], "operators": [], "source_path": None}
        if prefs:
            for k in dir(prefs):
                if k.startswith("_") or callable(getattr(prefs, k, None)):
                    continue
                try:
                    result["preferences"][k] = str(getattr(prefs, k))[:300]
                except Exception:
                    pass
        short = key.split(".")[-1]
        # Match by name
        for ns_name in dir(bpy.ops):
            if ns_name.startswith("_"):
                continue
            if short.lower() in ns_name.lower() or ns_name.lower() in short.lower():
                result["operator_namespaces"].append(ns_name)
        # Scan module source for bl_idname (handles Serpens addons etc.)
        import sys
        mod = sys.modules.get(key) or sys.modules.get("bl_ext.user_default." + short) or sys.modules.get(short)
        src_path = None
        if mod and hasattr(mod, "__file__") and mod.__file__:
            src_path = os.path.dirname(mod.__file__)
        else:
            # Try extensions folder
            ext_root = bpy.utils.user_resource("EXTENSIONS", path="user_default") if hasattr(bpy.utils, "user_resource") else None
            addon_root = bpy.utils.user_resource("SCRIPTS", path="addons") if hasattr(bpy.utils, "user_resource") else None
            search_roots = [r for r in (ext_root, addon_root) if r and os.path.isdir(r)]
            for root in search_roots:
                candidate = os.path.join(root, short)
                if os.path.isdir(candidate):
                    src_path = candidate
                    break
        result["source_path"] = src_path
        if src_path and os.path.isdir(src_path):
            bl_idnames = set()
            for fn in os.listdir(src_path):
                if not fn.endswith(".py"): continue
                try:
                    with open(os.path.join(src_path, fn), "r", encoding="utf-8", errors="ignore") as f:
                        for m in re.finditer(r'bl_idname\s*=\s*["\']([a-zA-Z0-9_\.]+)["\']', f.read()):
                            bl_idnames.add(m.group(1))
                except Exception: pass
            # Filter to operator-style ids (namespace.name)
            ops = sorted([b for b in bl_idnames if "." in b])
            result["operators"] = ops
            # Add discovered namespaces
            for op in ops:
                ns = op.split(".")[0]
                if ns not in result["operator_namespaces"]:
                    result["operator_namespaces"].append(ns)
        return result

    def get_addon_prefs(self, name):
        key = self._resolve_addon_key(name)
        if not key:
            return {"error": f"addon '{name}' non trovato"}
        prefs = bpy.context.preferences.addons[key].preferences
        return {k: str(getattr(prefs, k))[:300] for k in dir(prefs)
                if not k.startswith("_") and not callable(getattr(prefs, k, None))}

    def set_addon_prefs(self, name, **kwargs):
        key = self._resolve_addon_key(name)
        if not key:
            return {"error": f"addon '{name}' non trovato"}
        prefs = bpy.context.preferences.addons[key].preferences
    ...
    def get_scene_prop(self, name):
        s = bpy.context.scene
        if "." in name:
            # nested: es. 'stablegen_props.prompt'
            obj = s
            for part in name.split("."):
                obj = getattr(obj, part, None)
                if obj is None:
                    return {"error": f"prop '{name}' non trovata"}
            return {"name": name, "value": str(obj)[:500]}
        return {"name": name, "value": str(getattr(s, name, None))[:500]}

    def set_scene_prop(self, name, value):
        s = bpy.context.scene
        if "." in name:
            parts = name.split(".")
            obj = s
            for part in parts[:-1]:
                obj = getattr(obj, part, None)
                if obj is None:
                    return {"error": f"path '{name}' non valido"}
            setattr(obj, parts[-1], value)
        else:
            setattr(s, name, value)
        return {"ok": True, "name": name, "value": str(value)[:200]}

    def find_scene_props(self, pattern=""):
        """Trova scene props che contengono pattern (case-insensitive)."""
        s = bpy.context.scene
        pattern_low = pattern.lower()
        matches = {}
        for k in dir(s):
            if k.startswith("_") or callable(getattr(s, k, None)):
                continue
            if pattern_low and pattern_low not in k.lower():
                continue
            try:
                matches[k] = str(getattr(s, k))[:200]
            except Exception:
                pass
        return matches
